...
async function redrawFinal() {
  ...
  if (showKishiko) {
    ...
    ctx.lineWidth = 10;
    ctx.lineJoin = "round";
    ctx.strokeText("岸高41期生同窓会", x, y);

    ctx.fillStyle = "#ff7aa8";
    ctx.fillText("岸高41期生同窓会", x, y);
  }
...
async function redrawTextLayer() {
  ...
  if (showKishiko) {
    ...
    textCtx.strokeText("岸高41期生同窓会", w / 2, y);

    textCtx.fillStyle = "#ff7aa8";
    textCtx.fillText("岸高41期生同窓会", w / 2, y);
  }
...
shutterBtn.onclick = async () => {
  ...
  try {
    ...
    userMessage = "人が写っていないみたい 🌸\nカメラに人が映るようにして、もう一度撮ってね！";
    }
...
async function renderLight() {
  ...
  if (showKishiko || showPlace) {
      ...
      if (showKishiko) {
        ...
        ctx.strokeStyle = "#fff";
        ctx.fillStyle = "#ff7aa8";
        ctx.strokeText("岸高41期生同窓会", frameW / 2, 20);
        ctx.fillText("岸高41期生同窓会", frameW / 2, 20);
      }
